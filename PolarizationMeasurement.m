function I = PolarizationMeasurement(DipoleVector,v_alpha)
	...
	% tphi = 0:0.01:2*pi;
	% ttheta = 0:0.01:asin(n_2/n_1);
	% [tphi,ttheta] = meshgrid(tphi,ttheta);
	% figure(2);
	% r = arrayfun(@(theta_1,phi) abs(sum(E(theta_1,phi).*v_alpha))^2*n_1,ttheta,tphi);
	% surf(cos(tphi).*sin(ttheta).*r,sin(tphi).*sin(ttheta).*r,cos(ttheta).*r,r,'LineStyle','none');
	% colorbar;
	% axis equal;
