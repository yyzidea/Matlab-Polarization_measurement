function ROI = QDPLTraceExtraction(data,ROI_center,varargin)
	...
	for FrameIndex = 1:length(ROI)
		frame = squeeze(data(FrameIndex,:,:));

		% ROI_center = [119,83];

		ROI_range = sum(sum(frame(ROI_center(2)-ROI_size/2:ROI_center(2)+ROI_size/2,ROI_center(1)-ROI_size/2:ROI_center(1)+ROI_size/2)));
		ROI_background = sum(sum(frame(ROI_center(2)-ROI_size/2-ROI_border_size:ROI_center(2)+ROI_size/2+ROI_border_size,ROI_center(1)-ROI_size/2-ROI_border_size:ROI_center(1)+ROI_size/2+ROI_border_size)));
		ROI_background = (ROI_background - ROI_range)/((ROI_size+ROI_border_size*2+1)^2-(ROI_size+1)^2);
		ROI(FrameIndex) = ROI_range - ROI_background*(ROI_size+1)^2;
		...
		if DEBUG
			...
			if DEBUG_LOG
				fprintf('Debug info\nCurrent frame: %d/%d\nROI center:(%d,%d)\nROI_background: %f\nROI_range: %f\nROI: %f\n',...
					FrameIndex,length(ROI),ROI_center(1),ROI_center(2),ROI_background,ROI_range/(ROI_size+1)^2,ROI(FrameIndex));
			end
